      ...
      return class Nekos extends Plugin {
        ...
        async getGif(send, emotion) {
          ...
          if(emotion == undefined){
            ...
            `https://tenor.googleapis.com/v2/search?random=true&q=nekos&key=AIzaSyDy2fnAUloDkGFCC1IEtRkcqrFxAfLqB_Q&limit=20`
          );
          }
          else{
            response = await fetch(
              `https://tenor.googleapis.com/v2/search?random=true&q=nekos ${emotion}&key=AIzaSyDy2fnAUloDkGFCC1IEtRkcqrFxAfLqB_Q&limit=20`
          );
          }
          if (!response.ok){ 
